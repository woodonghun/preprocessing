...
def load_json(path: str):
    ...
    list2 = list(itertools.chain(*json_data.values()))      # 2중 list 형태를 하나의 list 로 생성
...
def load_xlsx(path: str):
    ...
    get_cells = ws['A5': f'C{row_max}']     # 출력할 cell 범위 지정 => row_max 까지 사용하지는 않지만 범위를 정해야 하기 때문에 사용
    for row in get_cells:

        for cell in row:
            if cell.value == 'Next Id':     # next id 가 있으면 끝
                break

            elif 'B' in cell.coordinate:    # column = B ( landmark_num )
                # print(cell.value)
...
def compare_json_xlsx(json_land_list: list, xlsx_land_dict: dict):
    ...
    print(f'json 에만 존재하는 번호 list : {only_json}\nxlsx 에만 존재하는 번호 list : {only_xlsx}')
