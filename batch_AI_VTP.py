...
use_gpu = 'temp8'  # 8 GPU 를 사용할 것 인지 정하는 옵션 ( 0 or 1 )  args[6]

mha_root = r'D:/538ons/export_volume_template/input'  # 추론 할 image 폴더가 모여 있는 폴더 경로
batch_loc = 'C:/Users/3DONS/Desktop/1'  # batch 파일을 생성할 폴더 경로
predict_number = 512  # 하나의 배치 파일 안에 predict 할 개수
# 나머지 개수에 때라 predict_number 의 개수가 증가할 수 있음
# ( 총 배치 파일 개수 = predict 총 개수 / batch number )
...
# 배치 파일 생성
def make_batch(id_mha_dict: dict, root_loc: str, bat_loc: str, pre_num: int, *args):
    total_predict = len(id_mha_dict)
    ...
    insert_predict = rest_predict // (total_predict // pre_num)  # batch 하나당 추가 하는 predict 개수
    insert_rest_predict = rest_predict % (total_predict // pre_num)  # batch 에 한개씩 추가 하는 나머지 predict 개수

    print(f' 총 predict 개수 : {total_predict}')
    print(f' 총 batch 개수 : {total_predict // pre_num}')
    print(f' batch 한 개당 predict 개수 : {pre_num} 남은 개수 : {rest_predict}')
    print(f' 남은 개수 batch 에 {insert_predict}개 씩 추가, 남은 {insert_rest_predict}개 앞에 하나씩 추가')
    ...
    print(f' 생성되는 perdict 개수 {pre_in_batch}')
...
make_batch(mha_dict, mha_root, batch_loc, predict_number, file_name, key, result_report_path, root_dir, model_dir, output_path, use_gpu)
